...
const FullTimeHotlist = React.memo(() => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const result = await hotlistAPI.getFullTimeHotlist(params);
    ...
    (row) => {
      const deleteConsultantAction = async () => {
        try {
          ...
          showSuccessToast(result.message || "FullTime Consultant deleted ");
          setRefreshKey((prev) => prev + 1);
        } catch (error) {
          console.error("Delete error:", error);
          showErrorToast("Failed to delete FullTime consultant ");
        }
      };
      showDeleteConfirm(deleteConsultantAction, row.name || "this FullTime consultant");
    },
    [userId]
  );

  const handleNavigate = (consultantId) => {
    navigate(`/dashboard/hotlist/full-time/${consultantId}`);
  };
      ...
      {!showCreateForm ? (
        <>
          ...
          <CustomDataTable
            ...
            loading={loading}
            filters={filters}
            filterStorageKey="fulltime_hotlist_filters"
...
export default FullTimeHotlist;