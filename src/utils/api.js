...
api.interceptors.request.use(
  (config) => {
    ...
    // above must NOT be sent. Multipart requests need a
    // "multipart/form-data; boundary=----WebKitFormBoundary..." header,
    // and that boundary can only be generated by the browser/XHR — which
    // it will only do automatically if no Content-Type header is present
    // at all. Explicitly setting "multipart/form-data" (with no boundary)
    // is just as broken as leaving "application/json": either way the
    // server can't parse the multipart body, and the connection can fail
    // outright instead of returning a clean error response.
    //
    // So: if this request's data is FormData, strip any Content-Type
    // header entirely and let the browser attach the correct one with
    // boundary included.
...
// ========== Form Data Methods ==========
// NOTE: Content-Type is intentionally NOT set here. The request
// interceptor above strips any Content-Type header whenever the payload
// is a FormData instance, which lets the browser/XHR generate the
// required "multipart/form-data; boundary=..." header automatically.
// Setting Content-Type manually (even to "multipart/form-data") prevents
// the boundary from being attached and breaks parsing on the server.
...
export const hotlistAPI = {
  ...
  // Update consultant
  // FIXED: this previously called apiPut (JSON content-type) even though
  // consultantDto is a FormData instance containing files. That mismatch
  // is why edits never sent resumes/documents correctly and could fail
  // with no clean HTTP response at all. Now correctly uses
  // apiPutFormData, and isAssignAll is passed through as a query param
  // (mirroring how createConsultant passes its query params) since it
  // is not a file and doesn't belong in the multipart body.
  updateConsultant: async (consultantId, consultantDto, isAssignAll = false) => {
    if (!consultantId) throw new Error("Consultant ID is required");
    if (!consultantDto) throw new Error("Consultant data is required");

    const queryString = new URLSearchParams({
      isAssignAll: String(isAssignAll),
    }).toString();

    return apiPutFormData(
      `/hotlist/updateConsultant/${consultantId}?${queryString}`,
      consultantDto
    );
  },
